...
library(ggplot2)
setwd("/wellstein/user_homes/il1380/project/6go/")
############################################
# 1. 读取 DESeq2 结果
############################################

res_df <- read.csv("../5clustering/DE_results.csv", row.names = 1)
...
deg <- res_df[
  res_df$padj < threshold_p & abs(res_df$log2FoldChange) > threshold_fc,
]
...
gene_nodes <- data.frame(
  id = as.character(node_df$id_entrez),
  label = node_df$id,          # SYMBOL
  log2FC = node_df$log2FC,
  padj = node_df$padj,
  type = "gene"
)
